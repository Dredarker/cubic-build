...
const badwords = [".[б6][аеу]", "шлю.", ".ля", ".[уy][яй]", "чл.н", "п..д", "п.д", "[сc][оo][сc].", "т[рp].[хxh]", "гн."];
			...
			if (data.type === "join") {
				for (const [id, clientData] of clients.entries()) {
					if (clientData.ws === ws) {
						...
						objects.set(id, new Player(nickname, 1.4, -11, new Obj(0, 0, 24, 80, "dynamic", "player", data.color)));
